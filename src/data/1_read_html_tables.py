...
# Adjust BASE_DIR to point to the root of the 'llm-glass-testes' project.
# If this script resides in 'llm-glass-testes/data/patents', move up three directories.
...
# Load desired compounds list from a JSON file using a relative path
properties_file = BASE_DIR / "json" / "properties.json"
try:
    with properties_file.open(encoding="utf-8") as f:
        properties_data = json.load(f)
        # Retrieve and normalize each compound name for matching
        desired_compounds = [normalize_string(compound) for compound in properties_data.get("desired_compounds", [])]
except (FileNotFoundError, json.JSONDecodeError) as e:
    # Report error if file is missing or contains invalid JSON
    print(f"Error loading properties.json file: {e}")
    desired_compounds = []

# Define the directory containing patent JSON files
json_folder = BASE_DIR / "data" / "patents"
...
for json_file in json_files:
    try:
        # Open and parse the JSON file content
    ...
    # Ensure the JSON contains an 'html_tables' field
    if not data.get("html_tables"):
        print(f"O arquivo {json_file.name} não contém o campo 'html_tables'.")
        continue

    # Iterate through each HTML table block in the JSON data
