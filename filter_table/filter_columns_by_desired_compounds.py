...
for coluna in desired_glasses_df1:
    # Linhas com valores numéricos (composições)
    numerical_values[coluna] = filtered_dataframe1[coluna].dropna()[filtered_dataframe1[coluna].apply(lambda x: isinstance(x, (int, float)))].tolist()

    # Linhas com "percent" ou "%" em qualquer parte do conteúdo (somente se for string)
    if filtered_dataframe1[coluna].dtype == 'object':  # Verifica se a coluna contém strings
        percent_values[coluna] = tuple(filtered_dataframe1[coluna][filtered_dataframe1[coluna].str.contains(r"\bpercent\b|\%", case=False, na=False)].tolist())
    else:
        percent_values[coluna] = ()  # Se não for string, armazena tupla vazia

    # Linhas com NaN e valores restantes que não são "percent" ou "%"
    other_values[coluna] = filtered_dataframe1[coluna][
        filtered_dataframe1[coluna].isna() | ~filtered_dataframe1[coluna].astype(str).str.contains(r"\bpercent\b|\%", case=False, na=False)
    ].tolist()
